...
var Client = module.exports = function(config) {
    ...
    if (config.proxy) {
        if (config.url.startsWith('https')) {
            this.proxyAgent = new HttpsProxyAgent(config.proxy)
        } else if (config.url.startsWith('http')) {
            this.proxyAgent = new HttpProxyAgent(config.proxy)
        }
    }
...
Client.prototype.request = function(opts, res) {
    ...
    var reqOpts = {};
    utils.mixin(reqOpts, opts)
    reqOpts.method = reqOpts.method || 'GET';
    if(!opts.url) {
        utils.mixin(reqOpts, utils.shallowCopy(this.config.url));
        // This doesn't seem to make much sense - why join the paths?
    ...
    if (opts.body && (reqOpts.method == 'POST' || reqOpts.method == 'PUT'))
        req.write(opts.body);
    req.end();
